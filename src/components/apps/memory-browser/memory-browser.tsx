...
export function MemoryBrowser() {
  ...
  const [selectedFile, setSelectedFile] = useState<MemoryFile | null>(
    mockMemoryFiles[0]
  );
  ...
  return (
    <div className="flex h-full bg-zinc-950">
            ...
            <AnimatePresence mode="wait">
              ...
              {viewMode === 'search' && (
                <SearchResults
                  ...
                  query={searchQuery}
                  onSelect={(result) => {
                    const file = mockMemoryFiles.find(
                      (f) => f.path === result.path
                    );
                    if (file) {
                      setSelectedFile(file);
                      setSelectedDaily(null);
              ...
              {viewMode === 'diff' && <DiffList diffs={mockDiffs} />}
            </AnimatePresence>
          </div>
        </ScrollArea>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <ScrollArea className="flex-1">
          <div className="p-4">
            {selectedFile && <MarkdownViewer content={selectedFile.content} />}
            {selectedDaily && <MarkdownViewer content={mockDailyContent} />}
        ...
        >
          ...
          {diff.oldContent && (
            <div className="mt-2 p-2 rounded bg-zinc-900 font-mono text-xs">
              ...
              <div className="text-green-400/70">+ {diff.newContent}</div>
            </div>
          )}
        </div>
      ))}
